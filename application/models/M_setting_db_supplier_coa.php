...
{
    $this->db->where('ID', $id);
    return $this->db->update('SETTING_DB_SUPPLIER_COA', $data);
}
  ...
  public function select($from_date,$to_date)
  {
    ...
    $this->db->from('SETTING_DB_SUPPLIER_COA');

    $this->db->join('AK_M_COA', 'AK_M_COA.ID = SETTING_DB_SUPPLIER_COA.COA_ID', 'left');
  ...
  {
    $this->db->where('ID',$id);
    $this->db->delete('SETTING_DB_SUPPLIER_COA');
  }

  function tambah($data)
  {
        $this->db->insert('SETTING_DB_SUPPLIER_COA', $data);
        return TRUE;
  }
