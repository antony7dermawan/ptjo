...
class c_t_ak_jurnal_print extends MY_Controller
{
  ...
  public function index($created_id)
  {
    ...
    for($i=0;$i<=$total_akun;$i++)
    {
      ...
      $pdf->SetFont('','',12);
      $pdf->Cell(25, 12, $no_akun[$i], 'L', 0, 'C');
      $pdf->MultiCell(50, 12, $nama_akun[$i], 'L', 'C',0,0);
      $pdf->Cell(30, 12, number_format(intval($debit[$i])), 'L', 0, 'C');
      $pdf->Cell(30, 12, number_format(intval($kredit[$i])), 'L', 0, 'C');

      $pdf->SetFont('','',10);
      $pdf->MultiCell(50, 12, $catatan[$i], 'L', 'C',0,0);
    ...
    $pdf->MultiCell(160, 6,$catatan[$total_akun], 1, 'L',0,1);
        ...
        $pdf->Cell(40, 12, "Dibuat:", 0, 0, 'C');
        $pdf->Cell(5, 12, "", 0, 0, 'C');
        $pdf->Cell(40, 12, "Diperiksa:", 0, 0, 'C');
        $pdf->Cell(5, 12, "", 0, 0, 'C');
        $pdf->Cell(40, 12, "Disetujui Oleh:", 0, 0, 'C');
        $pdf->Cell(5, 12, "", 0, 0, 'C');
        $pdf->Cell(40, 12, "Diketahui Oleh:", 0, 1, 'C');
        ...
        $pdf->Cell(40, 12, "Tgl:", 'T', 0, 'C');
        $pdf->Cell(5, 12, "", 0, 0, 'C');
        $pdf->Cell(40, 12, "Tgl:", 'T', 0, 'C');
        $pdf->Cell(5, 12, "", 0, 0, 'C');
        $pdf->Cell(40, 12, "Tgl:", 'T', 0, 'C');
        $pdf->Cell(5, 12, "", 0, 0, 'C');
        $pdf->Cell(40, 12, "Tgl:", 'T', 1, 'C');
